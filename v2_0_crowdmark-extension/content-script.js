...
class CrowdmarkProcessor {
    ...
    async processFullDocument() {
        ...
        try {
            ...
            const parsedData = {  ids: ocrResult.results || [],names: [],rawText: ''};
...
async matchSingleStudent(studentId) {
    ...
    // Count results in the dropdown
    const resultsList = document.querySelector('.manual-matching-search__results');
    if (!resultsList) {
        console.error('Results list not found');
        return false;
    }

    const results = resultsList.querySelectorAll('li button.manual-matching-search__result');
    console.log(`Found ${results.length} results for ${studentId}`);

    if (results.length === 1) {
        // Single result — click the result button then the checkbox
        results[0].click();
        await this.delay(300);
        
        const checkbox = document.querySelector('.manual-matching-search__checkbox');
        if (checkbox) {
            checkbox.click();
            console.log(`Auto-selected student: ${studentId}`);
            return true;
        }
    } else if (results.length === 0) {
        console.log(`No results found for ${studentId}`);
        return false;
    } else {
        console.log(`Multiple results for ${studentId}, manual selection required`);
        return false;
    }
